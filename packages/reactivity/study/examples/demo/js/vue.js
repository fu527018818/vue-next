    ...
    if (!vm.$options.render) {
      ...
      vm.$options.render = createEmptyVNode // 先给render属性赋值一个空的vnode节点
      {
        ...
        if (
          ...
          // 上面条件不符合, 则vue猜测并提示安装组件失败, 模板或者渲染函数未定义
          warn(
    ...
    function updateChildren(
      ...
      while (oldStartIdx <= oldEndIdx && newStartIdx <= newEndIdx) {
        if (isUndef(oldStartVnode)) {
          // oldStartVnode没有,则oldStartIdx后移一位
          oldStartVnode = oldCh[++oldStartIdx] // Vnode has been moved left
        } else if (isUndef(oldEndVnode)) {
          // oldEndVnode没有,则oldEndIdx前移一位
          oldEndVnode = oldCh[--oldEndIdx]
        } else if (sameVnode(oldStartVnode, newStartVnode)) {
          // 处理 头部 的同类型节点，即oldStartVnode和newStartVnode指向同类节点的情况
          patchVnode(oldStartVnode, newStartVnode, insertedVnodeQueue)
          oldStartVnode = oldCh[++oldStartIdx]
          newStartVnode = newCh[++newStartIdx] // 标记: oldStartIdx 和 newStartIdx 后移1位
        } else if (sameVnode(oldEndVnode, newEndVnode)) {
          // 处理 尾部 的同类型节点，即oldEndVnode和newEndVnode指向同类节点的情况
          patchVnode(oldEndVnode, newEndVnode, insertedVnodeQueue)
          oldEndVnode = oldCh[--oldEndIdx]
          newEndVnode = newCh[--newEndIdx] // 标记: oldEndIdx 和 newEndIdx 前移1位
        } else if (sameVnode(oldStartVnode, newEndVnode)) {
          // Vnode moved right 处理 头尾 同类型节点，即oldStartVnode和newEndVnode指向同类节点的情况
          ...
          canMove &&
            ...
            ) // 把oldStartVnode的el节点元素右移到oldEndVnode的el节点后面紧跟节点元素的前面(说白了就是oldEndVnode的el节点的后面)
          oldStartVnode = oldCh[++oldStartIdx]
          newEndVnode = newCh[--newEndIdx] // 标记: oldStartIdx 后移1位, newEndIdx 前移1位
        } else if (sameVnode(oldEndVnode, newStartVnode)) {
          // Vnode moved left 处理 尾头 的同类型节点，即oldEndVnode和newStartVnode指向同类节点的情况
          patchVnode(oldEndVnode, newStartVnode, insertedVnodeQueue)
          canMove &&
            nodeOps.insertBefore(parentElm, oldEndVnode.elm, oldStartVnode.elm) // 把oldEndVnode的el节点左移到oldStartVnode的el节点的前面
          oldEndVnode = oldCh[--oldEndIdx]
          newStartVnode = newCh[++newStartIdx] // 标记: oldEndIdx 前移1位  newStartIdx 后移1位
        } else {
          if (isUndef(oldKeyToIdx)) {
            oldKeyToIdx = createKeyToOldIdx(oldCh, oldStartIdx, oldEndIdx)
          } // oldKeyToIdx一个对象,存储剩下oldCh的键值对: {key: 索引}
          idxInOld = isDef(newStartVnode.key)
            ? oldKeyToIdx[newStartVnode.key]
            : findIdxInOld(newStartVnode, oldCh, oldStartIdx, oldEndIdx) // 该三目是为了查找newStartVnode在oldCh的索引
          if (isUndef(idxInOld)) {
            // New element undefined的话，说明newStartVnode未在oldCh中找到,说明它是一个新增的节点,则创建一个新的节点,且插入到oldStartVnode的elm dom的前面
            createElm(
              newStartVnode,
              insertedVnodeQueue,
              parentElm,
            ...
            // 找到newStartVnode在oldCh中的索引,说明是需要处理更新的节点
            vnodeToMove = oldCh[idxInOld] // 在oldCh中找到需要移动的vnode
            if (sameVnode(vnodeToMove, newStartVnode)) {
              // 如果需要移动的vnode和newStartVnode是同类节点,则把vnodeToMove的el dom移动到oldStartVnode的el dom的前面，且把刚才移动的vnode标记为undefined
      ...
      if (oldStartIdx > oldEndIdx) {
        // 当oldCh中的起止点相遇了，但是新vnode中的起止点没有相遇，这时需要对新vnode中的未处理节点进行处理，这类节点属于更新中被加入的节点，需要将他们插入到DOM树中
        ...
        // 当新vnode中的起止点相遇,且newStartIdx超过newEndIdx,需要把oldCh中oldStartIdx和oldEndIdx之间(包含他们)的dom删除(但是oldCh还有undefined标记的则不需要删除)
    ...
    if (!options.render) {
      ...
      if (template) {
        if (typeof template === 'string') {
          ...
          // 如果 template 是 dom 的话，则使用dom的innerHTML 作为 模板
          template = template.innerHTML
        } else {
          // 如上都不行的话,提示失效的template选项
          {
        ...
        // 如果没有定义template模板,怎使用el dom 的 outerHTML 作为tempalte 模板
